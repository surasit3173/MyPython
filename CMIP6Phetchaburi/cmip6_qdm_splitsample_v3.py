...
class Config:
    ...
    # เกณฑ์ตรวจจับอนุกรมที่ถูกทำให้เรียบ (ใช้กับคอลัมน์ flag_smoothed เท่านั้น)
    flag_lag1: float = 0.45
    flag_wetfreq: float = 0.55
    # ── เกณฑ์จำแนกชั้นคุณภาพสถานี (Tier) ──────────────────────────────
    # เกณฑ์ที่ 1: ความสมเหตุสมผลของการแจกแจงรายวัน
    tier_sdii_min: float = 8.0          # mm/วันฝนตก ต่ำกว่านี้ = น่าจะแจกแจงจากค่าสะสม
    tier_lag1_max: float = 0.30         # ACF(1) สูงกว่านี้ = อนุกรมเรียบผิดปกติ
    tier_maxdaily_frac: float = 0.50    # ค่าสุดขั้วต้อง >= สัดส่วนนี้ของมัธยฐานเครือข่าย
    # เกณฑ์ที่ 2: ความสม่ำเสมอเชิงเวลาคร่อมรอยต่อ cal/val
    tier_pettitt_p: float = 0.05        # p ของ Pettitt ต่อ zero-fraction
    tier_wetfreq_change: float = 0.20   # การเปลี่ยนแปลงสัมพัทธ์ของ wet-day frequency
    save_pdf: bool = False
...
def classify_stations(qc: pd.DataFrame, pc: pd.DataFrame,
    ...
    d["ok_distribution"] = ((d["SDII"] >= cfg.tier_sdii_min) &
                            (d["lag1_acf"] < cfg.tier_lag1_max) &
                            (d["max_daily"] >= cfg.tier_maxdaily_frac * med_max))
    d["ok_homogeneity"] = ((d["pettitt_p"] > cfg.tier_pettitt_p) &
                           (d["wetfreq_rel_change"] < cfg.tier_wetfreq_change))
    ...
    def _why(r):
        why = []
        if r["SDII"] < cfg.tier_sdii_min:
            why.append(f"SDII={r['SDII']:.1f} < {cfg.tier_sdii_min}")
        if r["lag1_acf"] >= cfg.tier_lag1_max:
            why.append(f"lag1={r['lag1_acf']:.2f} >= {cfg.tier_lag1_max}")
        if r["max_daily"] < cfg.tier_maxdaily_frac * med_max:
            why.append(f"max={r['max_daily']:.0f} mm < {cfg.tier_maxdaily_frac:.0%} "
                       f"ของมัธยฐาน ({med_max:.0f})")
        if r["pettitt_p"] <= cfg.tier_pettitt_p:
            why.append(f"Pettitt p={r['pettitt_p']:.3f}")
        if r["wetfreq_rel_change"] >= cfg.tier_wetfreq_change:
...
def threshold_diagnostics(tiers: pd.DataFrame, cfg: Config = CFG) -> None:
    """
    ช่วยตัดสินว่าเกณฑ์ที่ตั้งไว้เหมาะกับเครือข่ายสถานีชุดนี้หรือไม่
    หลักการ: เกณฑ์ที่ดีควรตกลงใน 'ช่องว่าง' ระหว่างกลุ่ม ไม่ใช่กลางกลุ่มที่ค่าเกาะกันแน่น
    รายงานจุดตัดตามธรรมชาติ (largest gap) เพื่อเทียบกับเกณฑ์ปัจจุบัน
    """
    specs = [("SDII", cfg.tier_sdii_min, "ge", "--sdii-min"),
             ("lag1_acf", cfg.tier_lag1_max, "lt", "--lag1-max"),
             ("wetfreq_rel_change", cfg.tier_wetfreq_change, "lt", "--wetfreq-change")]
    print("\n[Threshold diagnostics] ตรวจว่าเกณฑ์ตกในช่องว่างระหว่างกลุ่มหรือไม่")
    for col, thr, direction, flag in specs:
        v = tiers[col].dropna().sort_values()
        if v.size < 3:
            continue
        gaps = v.diff().fillna(0)
        gi = int(gaps.values.argmax())
        natural = float((v.iloc[gi] + v.iloc[gi - 1]) / 2) if gi > 0 else float(v.iloc[0])
        gap = float(gaps.iloc[gi])
        rng = float(v.iloc[-1] - v.iloc[0]) or 1.0
        n_pass = int((v >= thr).sum() if direction == "ge" else (v < thr).sum())

        # เกณฑ์ปัจจุบันอยู่ห่างจากค่าที่ใกล้ที่สุดเท่าไร
        margin = float(np.min(np.abs(v.values - thr)))
        verdict = ("ตกในช่องว่าง" if margin >= 0.15 * gap and gap > 0.10 * rng
                   else "⚠ ตกใกล้/กลางกลุ่มข้อมูล ตัดสินใจตามอำเภอใจ")
        print(f"\n  {col}: เกณฑ์ปัจจุบัน {thr}  ({flag})  → ผ่าน {n_pass}/{v.size} สถานี")
        print(f"    ค่าเรียงลำดับ: {', '.join(f'{x:.2f}' for x in v.values)}")
        print(f"    ช่องว่างใหญ่สุด {gap:.3f} ({100*gap/rng:.0f}% ของพิสัย) "
              f"→ จุดตัดตามธรรมชาติ ≈ {natural:.2f}")
        print(f"    ระยะจากเกณฑ์ถึงค่าที่ใกล้ที่สุด {margin:.3f} → {verdict}")
    print("\n  หมายเหตุ: ปรับเกณฑ์ 'ก่อน' ดูผลการวิเคราะห์เท่านั้น")
    print("            การปรับหลังเห็นผลแล้วคือ selection bias ที่ป้องกันตัวเองไม่ได้\n")
...
def assert_no_leakage(cfg: Config = CFG) -> None:
    """
    เงื่อนไขที่บังคับคือ 'ห้ามซ้อนทับ' เท่านั้น
    ลำดับก่อนหลังไม่จำเป็น เพื่อให้รันแบบสลับช่วง (swapped split-sample)
    เป็น robustness test ได้ตามข้อเสนอของ Klemeš (1986)
    """
    c0, c1 = cfg.cal
    v0, v1 = cfg.val
    assert c0 <= c1 and v0 <= v1, "ช่วงเวลาไม่ถูกต้อง"
    overlap = max(0, min(c1, v1) - max(c0, v0) + 1)
    assert overlap == 0, (f"calibration {c0}-{c1} ซ้อนทับ validation {v0}-{v1} "
                          f"อยู่ {overlap} ปี")
    if c0 > v0:
        print(f"    [swapped split] fit {c0}-{c1} → validate {v0}-{v1} "
              "(ทดสอบความคงทนต่อการเลือกจุดแบ่ง)")
...
def cmd_inspect(folder: str, cfg: Config) -> None:
    ...
    threshold_diagnostics(tiers, cfg)
    print("พร้อมรัน --run แล้ว\n")
...
def cmd_run(folder: str, out: str, cfg: Config,
    ...
    with open(outdir / "run_config.json", "w", encoding="utf8") as f:
        json.dump({"cal": cfg.cal, "val": cfg.val, "wet_threshold": cfg.wet,
                   ...
                   "tier_thresholds": {
                       "sdii_min": cfg.tier_sdii_min, "lag1_max": cfg.tier_lag1_max,
                       "maxdaily_frac": cfg.tier_maxdaily_frac,
                       "pettitt_p": cfg.tier_pettitt_p,
                       "wetfreq_change": cfg.tier_wetfreq_change},
...
def main() -> None:
    ...
    ap.add_argument("--sdii-min", type=float, default=None,
                    help="เกณฑ์ SDII ขั้นต่ำ (mm/วันฝนตก) เริ่มต้น 8.0")
    ap.add_argument("--lag1-max", type=float, default=None,
                    help="เกณฑ์ ACF(1) สูงสุด เริ่มต้น 0.30")
    ap.add_argument("--maxdaily-frac", type=float, default=None,
                    help="สัดส่วนขั้นต่ำของค่าสุดขั้วเทียบมัธยฐานเครือข่าย เริ่มต้น 0.50")
    ap.add_argument("--pettitt-p", type=float, default=None,
                    help="เกณฑ์ p ของ Pettitt เริ่มต้น 0.05")
    ap.add_argument("--wetfreq-change", type=float, default=None,
                    help="การเปลี่ยนแปลงสัมพัทธ์สูงสุดของ wet-day frequency เริ่มต้น 0.20")
    ...
    CFG.boot_n = a.boot
    CFG.save_pdf = a.pdf
    for arg, attr in (("sdii_min", "tier_sdii_min"), ("lag1_max", "tier_lag1_max"),
                      ("maxdaily_frac", "tier_maxdaily_frac"),
                      ("pettitt_p", "tier_pettitt_p"),
                      ("wetfreq_change", "tier_wetfreq_change")):
