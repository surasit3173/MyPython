...
def plot_mmk(x, y, st, season, out, title):
    ...
    Z, p = res.z, res.p
    r1 = pd.Series(y).autocorr(lag=1)  # แก้ไขตรงนี้
...
def main():
    ...
    for f in files:
        ...
        for st in stations:
            ...
            for s_name, series in seasons.items():
                ...
                # Table 2: Standard MK
                table2.append([st, s_name, len(years), round(Z_mk, 4), 
                               round(p_mk, 6), trend_mk])

                # Table 3: Modified MK
                table3.append([st, s_name, round(pd.Series(values).autocorr(lag=1), 4), 
                               round(Z_mmk, 4), round(p_mmk, 6), trend_mmk])
    ...
    # ====================== EXPORT ======================
    df2 = pd.DataFrame(table2, columns=["Station", "Scale", "N", "Z", "p-value", "Trend"])
    df3 = pd.DataFrame(table3, columns=["Station", "Scale", "ρ₁", "Z", "p-value", "Trend"])
    ...
    print("   • p-value ถูกต้องตามหลักวิชาการ")
    print("   • แสดงทุกสถานี ทุกสเกลเวลา")
