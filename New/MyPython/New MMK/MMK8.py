...
"""
===============================================================================
Q2-Q3 Standard Rainfall Trend Analysis (Final Version)
- Standard MK + Modified MK (Hamed & Rao)
- Theil–Sen slope + 95% CI
- Seasonal (Annual / Wet / Dry)
- Table 2, Table 3, Table 5 (ตามตัวอย่างที่ต้องการ)
- กราฟแบบ MMK3.py (MK vs MMK แยก)
- เฉพาะสถานีที่มีนัยสำคัญ (p < 0.05) ทั้ง 3 ฤดู
===============================================================================
"""
...
def main():
    ...
    for f in files:
        ...
        for st in stations:
            ...
            for s_name, series in seasons.items():
                ...
                # กรองเฉพาะสถานีที่มีนัยสำคัญในอย่างน้อย 1 ฤดู
                if p_mk >= 0.05 and p_mmk >= 0.05:
                    continue  # ข้ามสถานีที่ไม่มีนัยสำคัญทั้ง 3 ฤดู
                ...
                # Table 2: Standard MK
                table2.append([st, s_name, len(years), S, var_s, Z,
                               round(Z / (len(years)*(len(years)-1)/2), 4),
                               p_mk, trend_mk])

                # Table 3: Modified MK
                table3.append([st, s_name, var_mod, round(neff, 2), r1, Z_mmk,
                               round(Z_mmk / (len(years)*(len(years)-1)/2), 4),
                               p_mmk, trend_mmk])

                # Table 5: Summary
                table5.append([st, s_name, slope, p_mmk,
    ...
    print("   • กราฟ MK และ MMK แยกตามสถานีที่มีนัยสำคัญ")
