...
def _extract_model(fname):
    """Robustly extract model name from CMIP6 filename."""
...
def discover_files(folder):
    ...
    if len(obs) > 1:
        print(f"  ⚠  Multiple Observed files — using {obs[0].name}")

    # Province filter: only keep files matching observed province
    prov_kw = set()
    if obs_path:
...
def fig5_mme_added_value(obs_d, raw_dfs, bc_dfs, stns_str, smap,
    ...
    if all_kge_v:
        ...
        ax3.set_aspect("equal", "box")
    n_ens_wins_kge = sum(
        1 for stn in stns_str
        if not np.isnan(ev := metrics_from_dfs(obs_d, bc_ens, stn).get("KGE", np.nan))
        and not np.isnan(bv := max(
...
def write_word_report(obs_d, raw_dfs, bc_dfs, stns_str, smap,
    ...
    n_ens_wins = sum(
        1 for stn in stns_str
        if not np.isnan(ev := regional_metric(obs_d, bc_ens, [stn], "KGE"))
        and not np.isnan(bv := max(
