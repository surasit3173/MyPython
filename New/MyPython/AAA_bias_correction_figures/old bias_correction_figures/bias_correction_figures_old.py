...
def fig2_annual_timeseries(obs_d, raw_dfs: dict, bc_dfs: dict,
    ...
    fig.suptitle(
        f"Figure 2  Time series of annual rainfall during {period_obs} "
...
def fig3_probability_distribution(obs_d, raw_dfs: dict, bc_dfs: dict,
    ...
    """
    Figure 3: Probability Distribution of Daily Rainfall
    KDE for all wet-day values pooled across stations.
    Includes heavy-tail inset (> P95 of Observed).
    """
    stns_str = [str(s) for s in stns]
    ...
    for v, col, lbl, ls, lw in [
        (obs_v, C["obs"], "Observed",              "-",  2.6),
        (raw_v, C["raw"], "Raw CMIP6",             "--", 2.0),
        (bc_v,  C["bc"],  "Bias-Corrected (QDM)",  "-",  2.2),
    ]:
        ...
        ax.plot(x_grid, dens, color=col, ls=ls, lw=lw,
                label=f"{lbl}  (Skewness={sk:.2f}, Kurtosis={ku:.2f})")
        ax.fill_between(x_grid, dens, alpha=0.06, color=col)

    ax.set_xlabel("Daily Rainfall (mm)", fontsize=13, fontweight="bold")
    ax.set_ylabel("Probability Density", fontsize=13, fontweight="bold")
    ax.set_title(
        "Probability Density Function — Wet-Day Rainfall "
        f"(≥ {WET_THR} mm)  |  All Stations Pooled",
        loc="left", fontsize=13, fontweight="bold", pad=5
    )
    ax.set_xlim(0, x_max)
    ax.tick_params(axis="both", which="major", labelsize=11, width=1.5)
    ...
    # ── Heavy-tail inset (> P95 of Observed) ────────────────────────────
    p95_obs = float(np.percentile(obs_v, 95))
    p99_obs = float(np.percentile(obs_v, 99))
    x_tail  = np.linspace(p95_obs, x_max, 600)
    ax_ins  = ax.inset_axes([0.46, 0.34, 0.52, 0.58])

    for v, col, lbl, ls, lw in [
        (obs_v, C["obs"], "Observed",             "-",  2.0),
        (raw_v, C["raw"], "Raw CMIP6",            "--", 1.6),
        (bc_v,  C["bc"],  "Bias-Corrected (QDM)", "-",  1.8),
    ]:
        kde_t = gaussian_kde(v, bw_method="scott")
        ax_ins.plot(x_tail, kde_t(x_tail), color=col, ls=ls, lw=lw, label=lbl)
        ax_ins.fill_between(x_tail, kde_t(x_tail), alpha=0.07, color=col)
    ...
    ax_ins.set_xlabel("mm (tail)", fontsize=9)
    ax_ins.set_ylabel("Density",   fontsize=9)
    ax_ins.set_title("Heavy Tail (> P95)", fontsize=10, fontweight="bold")
    ax_ins.tick_params(labelsize=9)
    ax_ins.spines["top"].set_visible(False)
    ax_ins.spines["right"].set_visible(False)
    ax_ins.legend(fontsize=8.5, frameon=True, edgecolor="#B0BEC5",
                  loc="upper right")

    fig.suptitle(
        f"Figure 3  Probability distribution of daily rainfall for observed data, "
...
def fig4_qq_plots(obs_d, raw_dfs: dict, bc_dfs: dict,
    ...
    fig.suptitle(
        "Figure 4  Quantile–Quantile plots comparing rainfall distributions "
...
def _draw_taylor_background(ax, ref_std: float, unit: str) -> float:
    ...
    for frac, lc in [(0.25, "#CFD8DC"), (0.5, "#B0BEC5"),
        ...
        ax.plot(xc[mask], yc[mask], color=lc, lw=0.65, ls="--",
                alpha=0.75, zorder=1)
        idx = np.argmin(np.abs(theta - np.pi / 4))
        if mask[idx]:
            ax.text(xc[idx], yc[idx], f"RMSE\n{rr:.1f}",
                    fontsize=6.5, color=lc, ha="center", va="center",
                    bbox=dict(boxstyle="round,pad=0.1",
                              fc="white", ec="none", alpha=0.75))

    # Correlation arcs (lines from origin)
    for rv, lc in [(0.2, "#CFD8DC"), (0.4, "#B0BEC5"), (0.6, "#90A4AE"),
        ...
        ax.text(r_max * np.cos(tv) * 1.06,
                r_max * np.sin(tv) * 1.06,
                f"{rv:.2f}", fontsize=7, color="#546E7A",
                ha="center", va="center")
    ...
    for frac in [0.25, 0.5, 0.75, 1.0, 1.25, 1.5]:
        ...
        ax.text(0, ar, f"{ar:.1f}", fontsize=7,
                color="#90A4AE", ha="right", va="center")

    # Reference point
    ax.plot(ref_std, 0, "k*", markersize=14, zorder=9,
...
def fig5_taylor_diagram(obs_d, raw_dfs: dict, bc_dfs: dict,
    ...
    """
    Figure 5: Taylor Diagram — Daily / Monthly / Wet Season
    Raw (▲) and Bias-Corrected (●) per station.
    Arrow: Raw → Corrected (improvement direction).
    """
    stns_str  = [str(s) for s in stns]
    ...
    scales = [
        ("Daily",      obs_d,
         {m: df for m, df in raw_dfs.items()},
         {m: df for m, df in bc_dfs.items()},
         "mm day⁻¹", "(a)"),
        ("Monthly",    to_monthly(obs_d),
         {m: to_monthly(df) for m, df in raw_dfs.items()},
         {m: to_monthly(df) for m, df in bc_dfs.items()},
         "mm month⁻¹", "(b)"),
        ("Wet Season", to_seasonal_wet(obs_d),
         {m: to_seasonal_wet(df) for m, df in raw_dfs.items()},
         {m: to_seasonal_wet(df) for m, df in bc_dfs.items()},
         "mm yr⁻¹", "(c)"),
    ]

    fig, axes = plt.subplots(1, 3, figsize=(24, 9))
    fig.subplots_adjust(left=0.03, right=0.97,
                        top=0.88, bottom=0.10, wspace=0.28)

    for ai, (scale, o_df, r_dict, b_dict, unit, panel) in enumerate(scales):
        ax = axes[ai]

        # Compute reference std from Observed (mean across stations)
        stds = []
        for stn in stns_str:
            if o_df is not None and stn in (o_df.columns if o_df is not None else []):
        ...
        raw_xy = {}   # Station → (x, y) for Raw point

        for si, stn in enumerate(stns_str):
            col = stn_colors[si % len(stn_colors)]
            code = smap.get(stn, stn)

            for ds_dict, marker, ds_label in [
                (r_dict, "^", "Raw"),
                (b_dict, "o", "BC"),
            ]:
                # Use ensemble mean across models for each dataset
                ens_df = ensemble_mean(ds_dict)
                ...
                ax.scatter(xv, yv, color=col, marker=marker, s=95,
                           zorder=7, edgecolors="white",
                ...
                if ds_label == "Raw":
                    ...
                    ax.annotate(
                        ...
                        arrowprops=dict(
                            arrowstyle="->", color=col, lw=1.0, alpha=0.60
                        )
                    )
                    lx = xv + 0.026 * r_max
                    ly = yv + 0.026 * r_max
                    ax.text(lx, ly, code, fontsize=8.5, color=col,
                            fontweight="bold", ha="left", va="bottom")

        handles = [
            Line2D([0], [0], marker="*",  color="k",    ls="none",
                   ms=11, label="Observed (Ref.)"),
            Line2D([0], [0], marker="^",  color="grey", ls="none",
                   ms=9,  label="Raw CMIP6"),
            Line2D([0], [0], marker="o",  color="grey", ls="none",
                   ms=9,  label="Bias-Corrected (QDM)"),
            Line2D([0], [0], color="grey", lw=1.0, ls="-", alpha=0.6,
        ...
        ax.set_title(
            f"{panel}  Taylor Diagram — {scale}",
            loc="left", fontsize=13, fontweight="bold", pad=5
        )

    fig.suptitle(
        f"Figure 5  Taylor diagram showing the performance of CMIP6 rainfall "
        "simulations before and after bias correction.\n"
...
def fig6_metric_improvement(obs_d, raw_dfs: dict, bc_dfs: dict,
    ...
    fig.suptitle(
        "Figure 6  Improvement in statistical performance metrics "
