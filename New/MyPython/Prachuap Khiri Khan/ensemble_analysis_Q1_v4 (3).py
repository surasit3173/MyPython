"""
===============================================================================
  Multi-Model Ensemble Analysis  — Version 4.0
...
-------------------------------------------------------------------------------
  Changes v4.0  (based on v3.0):
    A. SMART PROVINCE EXTRACTION: รองรับชื่อจังหวัดที่มีเว้นวรรค เช่น
       "Prachuap Khiri Khan" โดยดึงคำทั้งหมดหลังส่วนวันที่ในชื่อไฟล์ Observed
       (ไม่ใช้ regex ที่ดึงได้แค่คำเดียว)
    B. DYNAMIC MODEL DISCOVERY: ดึงชื่อโมเดลจากชื่อไฟล์อัตโนมัติ
       ไม่ต้อง hardcode ชื่อโมเดลใน list
    C. UNIVERSAL FILE MATCHING: ใช้ชื่อจังหวัดเพื่อ filter ไฟล์
       รองรับเว้นวรรค underscore และตัวพิมพ์ใหญ่-เล็กผสม
    D. FIG 6 Panel A: ลบลำดับเหรียญ (#1, #2, #3) ออก แสดงแค่ชื่อโมเดล
    E. NEW ANALYSIS – Ensemble vs Best Single Model:
       วิเคราะห์ว่า Ensemble Mean มีประสิทธิภาพเหนือกว่า Best Single Model หรือไม่
       ผลแสดงใน Excel sheet "Ensemble vs Best Model"
    F. NEW ANALYSIS – QDM Trend Preservation:
       ตรวจสอบว่า QDM รักษาแนวโน้มฝน (trend signal) ของ Observed หรือไม่
       ใช้ Mann-Kendall test + Sen's slope
       ผลแสดงใน Excel sheet "QDM Trend Preservation"
===============================================================================
"""
...
# 0.  GLOBAL STYLE
# ═══════════════════════════════════════════════════════════════════════════
# ── Font size scale  (Q1–Q3 publication standard) ─────────────────────────
# Increase all sizes: title +3, label +2, tick +2, legend +3, annot +2, note +2
FS = dict(title=17, label=14, tick=13, legend=14, annot=12, note=10)
...
plt.rcParams.update({
    ...
    "font.size":            FS["label"],
    "font.weight":          "bold",          # default weight = bold
    ...
    "legend.edgecolor":      "#78909C",
    "legend.borderpad":      0.7,
    "legend.labelspacing":   0.55,
    "legend.handlelength":   2.8,       # longer colour swatch
    "legend.handleheight":   1.6,       # taller swatch  ← 2× previous 0.8
...
def _extract_province_from_obs(fname):
    """
    Extract province name from Observed filename that may contain spaces/underscores.
    Pattern examples:
    ...
    Strategy: strip known prefix tokens (Observed/Rain/daily + date-like tokens),
    then join remaining parts as province name.
    """
    stem = Path(fname).stem
    # Normalise: replace spaces with underscore for uniform splitting
    parts = stem.replace(" ", "_").split("_")

    # Drop tokens that are: "observed","rain","daily","monthly",
    # pure-digit (year/month codes), or empty
    skip_words = {"observed","rain","daily","monthly","data","station","rainfall"}
    province_parts = []
    skip_done = False
    for p in parts:
        low = p.lower()
        if not skip_done:
            if low in skip_words or re.fullmatch(r'\d+', p):
                continue
            else:
                skip_done = True
        if p:
            province_parts.append(p)

    # Reconstruct with spaces (Title Case each word)
    province = " ".join(w.capitalize() for w in province_parts)
    return province if province else None


def _get_model_name(fname):
    """
    Dynamically extract CMIP6 model name from filename.
    Handles:
      pr_day_CanESM5_historical_r1i1p1f1_gn_19810101...csv  → CanESM5
      bc_pr_day_ACCESSESM15_historical_...csv               → ACCESSESM15
      pr_day_EC-Earth3_...csv                               → EC-Earth3
    """
    # Pattern: (bc_)?pr_day_MODELNAME_ where MODELNAME may contain hyphens
    m = re.search(r'(?:bc_)?pr_day_([A-Za-z0-9][A-Za-z0-9\-\.]+?)_', fname, re.IGNORECASE)
    if m:
        return m.group(1)
    # fallback
    stem = Path(fname).stem
    parts = stem.split("_")
    # skip "bc","pr","day" prefixes
    skip = {"bc","pr","day"}
    for p in parts:
        if p.lower() not in skip and not re.fullmatch(r'\d+', p):
            return p
    return stem
...
def find_csvs_multi(folder):
    """
    Smart file discovery – supports province names with spaces.
    Returns:
        ...
        province  : str  (may contain spaces, e.g. "Prachuap Khiri Khan")
    """
    all_csv = sorted(Path(folder).glob("*.csv"))

    # ── Step 1: find Observed file & extract province ──────────────────
    obs_files = [f for f in all_csv if "observed" in f.name.lower()]
    if not obs_files:
        sys.exit("  ✗  ไม่พบไฟล์ Observed (ชื่อไฟล์ต้องมีคำว่า 'observed')")
    obs_path = str(obs_files[0])
    province = _extract_province_from_obs(obs_files[0].name)
    if not province:
        sys.exit("  ✗  ไม่สามารถดึงชื่อจังหวัดจากไฟล์ Observed ได้")
    print(f"  จังหวัด/พื้นที่ศึกษา: {province}")

    # ── Step 2: validate that ALL csv files share the same province ────
    # Use the province string (normalised) to check membership
    prov_norm = province.replace(" ", "").lower()
    mismatch  = []
    for f in all_csv:
        fname_norm = f.name.replace(" ", "").replace("_", "").lower()
        if prov_norm not in fname_norm:
            mismatch.append(f.name)
    if mismatch:
        print(f"  ⚠  ไฟล์ต่อไปนี้ไม่มีชื่อจังหวัด '{province}' ในชื่อไฟล์:")
        for fn in mismatch:
            print(f"       {fn}")
        print("  (ยังคงดำเนินการต่อโดยใช้ไฟล์ที่ตรงกัน)")

    # ── Step 3: find Raw and BC files dynamically ──────────────────────
    raw_files = [f for f in all_csv if f.name.lower().startswith("pr_")
                 and not f.name.lower().startswith("pr_obs")]
    bc_files  = [f for f in all_csv if f.name.lower().startswith("bc_")]

    raw_dict = {_get_model_name(f.name): str(f) for f in raw_files}
    bc_dict  = {_get_model_name(f.name): str(f) for f in bc_files}
...
# ═══════════════════════════════════════════════════════════════════════════
# 4b.  MANN-KENDALL + SEN'S SLOPE  (pure scipy, no external package)
# ═══════════════════════════════════════════════════════════════════════════
...
def mann_kendall(x):
    """
    Two-sided Mann-Kendall test (Mann 1945; Kendall 1975).
    Returns: (tau, p_value, S, z)
    """
    ...
    if   S > 0: z = (S - 1) / _math.sqrt(var_S)
    elif S < 0: z = (S + 1) / _math.sqrt(var_S)
    else:        z = 0.0
    p_val = 2.0 * (1.0 - _sps.norm.cdf(abs(z)))
    tau   = S / (0.5 * n * (n - 1))
    return float(tau), float(p_val), float(S), float(z)

def sens_slope_ci(x, years=None):
    """
    Sen's slope + 95% CI via scipy.stats.theilslopes.
    Returns: (slope, intercept, lo, hi)
    """
    x = np.asarray(x, dtype=float)
    if years is None:
    ...
    if mask.sum() < 4:
        return np.nan, np.nan, np.nan, np.nan
    res = _sps.theilslopes(x[mask], years[mask], 0.95)
...
# ═══════════════════════════════════════════════════════════════════════════
# 4c.  ENSEMBLE vs BEST SINGLE MODEL  (new analysis E)
# ═══════════════════════════════════════════════════════════════════════════

def analyse_ensemble_vs_best(model_perf, met_d_raw_ens, met_d_bc_ens,
                              stns_str, province):
    """
    Compare Ensemble Mean performance against the Best Single Model (QDM).
    Returns a list of result dicts for Excel output.
    """
    results = []

    # ── Step 1: per-model composite score (QDM) ───────────────────────
    def model_avg(model, key, mtype="bc"):
        lst = model_perf[model][mtype]
        vals = [x[key] for x in lst
                if not (isinstance(x.get(key, np.nan), float)
                        and np.isnan(x.get(key, np.nan)))]
    ...
    for mdl in models:
        ...
        for k in RANK_METRICS:
            all_v = [model_avg(m, k, "bc") for m in models]
            all_v = [v for v in all_v if not np.isnan(v)]
            v = model_avg(mdl, k, "bc")
        ...
        scores[mdl] = float(np.mean(sc))

    best_model   = max(scores, key=lambda m: scores[m])
    best_score   = scores[best_model]
    best_bc_rows = model_perf[best_model]["bc"]

    # ── Step 2: ensemble composite score ──────────────────────────────
    ens_rows = met_d_bc_ens
    ens_sc   = []
    for k in RANK_METRICS:
        # normalise ensemble vs best-model raw values
        ens_v  = float(np.nanmean([x[k] for x in ens_rows
                                   if not (isinstance(x.get(k, np.nan), float)
                                           and np.isnan(x.get(k, np.nan)))]))
        best_v = float(np.nanmean([x[k] for x in best_bc_rows
                                   if not (isinstance(x.get(k, np.nan), float)
                                           and np.isnan(x.get(k, np.nan)))]))
        both   = [v for v in [ens_v, best_v] if not np.isnan(v)]
        if len(both) < 2: ens_sc.append(0.5); continue
        mn, mx = min(both), max(both)
        rng = mx - mn
        if rng == 0: ens_sc.append(1.0)
        elif k in LOWER_BETTER: ens_sc.append((mx - ens_v) / rng)
        else:                   ens_sc.append((ens_v - mn) / rng)
    ens_composite = float(np.mean(ens_sc))

    # ── Step 3: per-metric comparison ─────────────────────────────────
    for k in RANK_METRICS:
        ens_v  = float(np.nanmean([x[k] for x in ens_rows
                                   if not (isinstance(x.get(k, np.nan), float)
                                           and np.isnan(x.get(k, np.nan)))]))
        best_v = float(np.nanmean([x[k] for x in best_bc_rows
                                   if not (isinstance(x.get(k, np.nan), float)
                                           and np.isnan(x.get(k, np.nan)))]))
        diff   = (best_v - ens_v) if k in LOWER_BETTER else (ens_v - best_v)
        winner = "Ensemble" if diff >= 0 else best_model
        results.append({
            "Metric":             k,
            "Ensemble Mean":      round(ens_v,  4) if not np.isnan(ens_v)  else "—",
            f"Best Model ({best_model})": round(best_v, 4) if not np.isnan(best_v) else "—",
            "Difference (Ens−Best)": round(ens_v - best_v, 4)
                                     if not (np.isnan(ens_v) or np.isnan(best_v)) else "—",
            "Winner":             winner,
            "Note":               "lower=better" if k in LOWER_BETTER else "higher=better",
        })

    summary = {
        "best_model":     best_model,
        "best_score_qdm": round(best_score, 4),
        "ens_composite":  round(ens_composite, 4),
        "ens_wins":       ens_composite >= best_score,
        "rows":           results,
        "all_scores":     {m: round(scores[m], 4) for m in
                           sorted(scores, key=lambda x: scores[x], reverse=True)},
    }
    return summary

# ═══════════════════════════════════════════════════════════════════════════
# 4d.  QDM TREND PRESERVATION  (new analysis F)
# ═══════════════════════════════════════════════════════════════════════════

def analyse_trend_preservation(obs_d, raw_ens_d, bc_ens_d,
                                model_perf, raw_dfs, bc_dfs,
                                stns_str, province):
    """
    Check whether QDM preserves the observed rainfall trend signal.
    Uses Mann-Kendall test + Sen's slope on annual totals.
    Returns list of dicts for Excel output.
    """
    def annual_stn(df, stn):
        s = df[stn].resample("YS").sum(min_count=int(0.8 * 365)).dropna()
        return s
    ...
    for stn in stns_str:
        for ds_lbl, df in [("Observed", obs_d),
                            ("Raw Ensemble", raw_ens_d),
                            ("QDM Ensemble", bc_ens_d)]:
            if stn not in df.columns: continue
            ann = annual_stn(df, stn)
            if len(ann) < 5: continue
            yrs  = ann.index.year.astype(float)
            vals = ann.values.astype(float)
            tau, p, S, z = mann_kendall(vals)
            sl, ic, lo, hi = sens_slope_ci(vals, yrs)
            rows.append({
                "Station":     stn,
                "Dataset":     ds_lbl,
                "N_years":     len(vals),
                "Mean (mm/yr)": round(float(np.mean(vals)), 2),
                "Sen Slope (mm/yr)": round(sl, 4) if not np.isnan(sl) else "—",
                "Slope 95% CI_lo":   round(lo, 4) if not np.isnan(lo) else "—",
                "Slope 95% CI_hi":   round(hi, 4) if not np.isnan(hi) else "—",
                "MK Tau":      round(tau, 4) if not np.isnan(tau) else "—",
                "MK Z":        round(z,   3) if not np.isnan(z)   else "—",
                "p-value":     round(p,   4) if not np.isnan(p)   else "—",
                "Significance":_sig_label(p),
                "Trend Direction": ("Increasing" if (not np.isnan(sl) and sl > 0)
                                    else ("Decreasing" if (not np.isnan(sl) and sl < 0)
                                          else "No trend")),
            })

    # Per-model trend rows
    models = list(model_perf.keys())
    for mdl in models:
        raw_df = raw_dfs[mdl]
        bc_df  = bc_dfs[mdl]
        for stn in stns_str:
            for ds_lbl, df in [(f"Raw {mdl}", raw_df),
                                (f"QDM {mdl}", bc_df)]:
                if df is None or stn not in df.columns: continue
                ann = annual_stn(df, stn)
                if len(ann) < 5: continue
                yrs  = ann.index.year.astype(float)
                vals = ann.values.astype(float)
...
def fig_boxplot_desc(desc_daily_raw_data, stns, smap,
    ...
    handles = [mpatches.Patch(facecolor=lt, edgecolor=col, linewidth=1.2,
                               alpha=0.85, label=lbl)
               for lbl, col, lt in zip(DS_LABELS, edge_cols, fill_cols)]
    fig.legend(handles=handles, loc="lower center", ncol=3,
               fontsize=FS["legend"], frameon=True, edgecolor="#78909C",
               bbox_to_anchor=(0.5, -0.01),
               handlelength=3.2, handleheight=1.8,
               borderpad=0.8, labelspacing=0.6, handletextpad=0.7)
...
def fig_performance(raw_rows, bc_rows, stns, smap, scale, title_suffix,
    ...
    for ai, (ylabel, key, higher, nse_thrs) in enumerate(metrics):
        ...
        if nse_thrs:
            for thr, lbl, _ in zip(nse_thrs,
                ...
                ax.axhline(thr, color="#9E9E9E", lw=0.9, ls="--", alpha=0.55)
                ax.text(n-0.5, thr+0.01, lbl,
                        fontsize=FS["annot"], color="#616161",
    ...
    handles = [
        mpatches.Patch(facecolor=C["raw_lt"], edgecolor=C["raw"],
                       linewidth=1.2, alpha=0.85, label="Raw CMIP6"),
        mpatches.Patch(facecolor=C["bc_lt"],  edgecolor=C["bc"],
                       linewidth=1.2, alpha=0.85, label="Bias-Corrected (QDM)"),
        Line2D([0],[0], color=C["green"], lw=1.8, ls=":", label="Perfect / Optimal"),
    ]
    fig.legend(handles=handles, loc="lower center", ncol=3,
               fontsize=FS["legend"], frameon=True, edgecolor="#78909C",
               bbox_to_anchor=(0.5, -0.01),
...
def fig_ranking(imp_daily, imp_monthly, rank_df, out_path,
    ...
    for bars, vals, col in [(b1,vals_d,C["bc"]),(b2,vals_m,C["gold"])]:
        for bar, v in zip(bars, vals):
            if not np.isnan(v):
                ...
                dy = 1.2 if v >= 0 else -1.2
                ax1.text(bar.get_x()+bar.get_width()/2, v+dy,
                         f"{v:+.1f}%", ha="center", va=va,
                         fontsize=FS["annot"], fontweight="bold", color=col)
    ...
    ax1.legend(fontsize=FS["legend"], frameon=True,
               edgecolor="#78909C", loc="upper right",
               handlelength=2.8, handleheight=1.6, borderpad=0.7)
    ...
    ax2.set_yticklabels(["0.25","0.50","0.75","1.00"],
                         fontsize=FS["annot"], color="#546E7A", fontweight="bold")
    ax2.set_title(
        "(b)  Normalised Performance Radar\n"
        "      (outer = better; QDM vs Raw CMIP6)",
        fontsize=FS["title"]-1, fontweight="bold", pad=20)
    ax2.legend(loc="lower right", bbox_to_anchor=(1.48, -0.14),
               fontsize=FS["legend"], frameon=True, edgecolor="#78909C",
               handlelength=2.8, handleheight=1.6, borderpad=0.7)
    ...
    for model, col in [("Raw CMIP6",C["raw"]),("Bias-Corrected (QDM)",C["bc"])]:
        ...
        ax2.text(0.02, 1.0-0.12*rk,
                 f"Rank {rk}: {model}\n  Composite Score = {cs:.4f}",
                 transform=ax2.transAxes,
                 fontsize=FS["annot"], color=col, fontweight="bold",
...
def fig_heatmap(raw_rows, bc_rows, stns, smap, out_path,
    ...
    fig, axes = plt.subplots(2, n_met, figsize=(n_met*5.2+1.5, 10),
                              gridspec_kw={"hspace":0.60,"wspace":0.28})
    fig.subplots_adjust(left=0.07, right=0.97, top=0.87, bottom=0.13)
                ...
                for ci, v in enumerate(vals[0]):
                    if not np.isnan(v):
                        ...
                        ax.text(ci, 0, f"{v:.3f}", ha="center", va="center",
                                fontsize=FS["annot"]+2, fontweight="bold",
                                color=txt_col)
    ...
    fig.legend(handles=nse_patches, loc="lower center", ncol=4,
               ...
               handlelength=3.2, handleheight=1.8, borderpad=0.8)
...
def fig_model_ranking(model_perf_dict, out_path, period_obs, province):
    ...
    ax2.set_title("(b)  Per-Model Normalised Performance Radar\n"
                  ...
                  fontsize=FS["title"]-1, fontweight="bold", pad=20)
    ax2.legend(loc="lower right", bbox_to_anchor=(1.52, -0.16),
               fontsize=FS["legend"]-1, frameon=True, edgecolor="#78909C",
               title="CMIP6 Model", title_fontsize=FS["legend"],
               handlelength=2.8, handleheight=1.6, borderpad=0.7)
    ax2.grid(True, lw=0.5, alpha=0.55)
...
# ═══════════════════════════════════════════════════════════════════════════
# 11b.  EXCEL – Ensemble vs Best Model sheet
# ═══════════════════════════════════════════════════════════════════════════

def _ensemble_vs_best_sheet(wb, summary, province):
    ws = wb.create_sheet("Ensemble vs Best Model")
    ws.sheet_view.showGridLines = False
    nc = 7

    best_model   = summary["best_model"]
    best_score   = summary["best_score_qdm"]
    ens_composite= summary["ens_composite"]
    ens_wins     = summary["ens_wins"]
    all_scores   = summary["all_scores"]
    rows_data    = summary["rows"]

    # ── Header ────────────────────────────────────────────────────────
    mxsc(ws,1,1,nc,
         f"Ensemble Mean vs Best Single Model — {province}",
         bold=True,fc="FFFFFF",bg=XC["title"],sz=12,align="left")
    ws.row_dimensions[1].height = 22

    verdict_bg  = XC["improve"] if ens_wins else XC["degrade"]
    verdict_txt = ("✔  Ensemble Mean OUTPERFORMS the best single model"
                   if ens_wins else
                   f"✘  Best single model ({best_model}) outperforms Ensemble Mean")
    mxsc(ws,2,1,nc, verdict_txt,
         bold=True, fc="FFFFFF",
         bg="1B5E20" if ens_wins else "B71C1C", sz=11, align="left")
    ws.row_dimensions[2].height = 20

    mxsc(ws,3,1,nc,
         f"Ensemble Composite Score: {ens_composite:.4f}   │   "
         f"Best Model ({best_model}) Score: {best_score:.4f}   │   "
         "Metric: QDM-corrected composite (normalised, 0=worst → 1=best)",
         italic=True, fc="FFFFFF", bg=XC["sub"], sz=8.5)
    ws.row_dimensions[3].height = 14

    # ── All model scores ──────────────────────────────────────────────
    row_cur = 5
    mxsc(ws,row_cur,1,3,"CMIP6 Model Composite Scores (QDM)",
         bold=True,fc="FFFFFF",bg=XC["hdr"],sz=10)
    ws.row_dimensions[row_cur].height = 18; row_cur+=1
    for rank_i, (mdl, sc) in enumerate(all_scores.items(), 1):
        bg = XC["rank1"] if rank_i==1 else (XC["rank2"] if rank_i==2
             else (XC["rank3"] if rank_i==3 else XC["white"]))
        xsc(ws,row_cur,1,f"Rank {rank_i}",bold=True,bg=bg,border=tb(),sz=10)
        xsc(ws,row_cur,2,mdl,bold=(rank_i==1),bg=bg,border=tb(),sz=10,align="left")
        xsc(ws,row_cur,3,sc,bg=bg,border=tb(),sz=10)
        ws.row_dimensions[row_cur].height = 16; row_cur+=1

    # Ensemble row
    ens_bg = XC["improve"] if ens_wins else XC["note"]
    xsc(ws,row_cur,1,"Ensemble",bold=True,bg=ens_bg,border=tb(),sz=10)
    xsc(ws,row_cur,2,"(Mean of all models)",italic=True,bg=ens_bg,border=tb(),sz=10,align="left")
    xsc(ws,row_cur,3,ens_composite,bold=True,bg=ens_bg,border=tb(),sz=10)
    ws.row_dimensions[row_cur].height = 16; row_cur+=2

    # ── Per-metric comparison ─────────────────────────────────────────
    mxsc(ws,row_cur,1,nc,
         "Per-Metric Comparison: Ensemble Mean vs Best Single Model",
         bold=True,fc="FFFFFF",bg=XC["sub"],sz=11)
    ws.row_dimensions[row_cur].height = 20; row_cur+=1
    ...
    for rd in rows_data:
        ...
        bg = XC["improve"] if winner=="Ensemble" else XC["degrade"]
        for ci,k in enumerate(col_names,1):
            v = rd.get(k,"")
            cell = xsc(ws,row_cur,ci,v,bg=bg,border=tb(),sz=10)
            if k=="Winner":
                fc_col = XC["obs_h"] if winner=="Ensemble" else XC["raw_h"]
                cell.font = Font(bold=True,color=fc_col,name="Calibri",size=10)
        ws.row_dimensions[row_cur].height = 16; row_cur+=1
    ...
    row_cur+=1
    mxsc(ws,row_cur,1,nc,
         "Interpretation: Ensemble Mean typically reduces model-specific biases "
         "by averaging out individual model errors (Tebaldi & Knutti 2007). "
         "A higher composite score indicates better overall agreement with observations. "
         "Green rows indicate Ensemble outperforms the best single model for that metric.",
         italic=True,fc=XC["sub"],bg=XC["note"],sz=8.5)
    ws.row_dimensions[row_cur].height = 40

    cw(ws,1,10); cw(ws,2,28); cw(ws,3,16); cw(ws,4,16)
    cw(ws,5,18); cw(ws,6,10); cw(ws,7,14)


# ═══════════════════════════════════════════════════════════════════════════
# 11c.  EXCEL – QDM Trend Preservation sheet
# ═══════════════════════════════════════════════════════════════════════════

def _trend_preservation_sheet(wb, trend_rows, province):
    ws = wb.create_sheet("QDM Trend Preservation")
    ws.sheet_view.showGridLines = False
    ws.freeze_panes = "C6"
    nc = 12

    mxsc(ws,1,1,nc,
         f"QDM Trend Preservation Analysis — {province}",
         bold=True,fc="FFFFFF",bg=XC["title"],sz=12,align="left")
    ws.row_dimensions[1].height = 22
    mxsc(ws,2,1,nc,
         "Mann-Kendall trend test on annual rainfall totals (YS resample). "
         "QDM is considered to 'preserve trend' if the trend direction and "
         "significance are consistent with Observed. "
         "Sen's slope = mm/year | *** p<0.001 ** p<0.01 * p<0.05 † p<0.10 ns p≥0.10",
         italic=True,fc="FFFFFF",bg=XC["sub"],sz=8.5)
    ws.row_dimensions[2].height = 20
    mxsc(ws,3,1,nc,
         "Reference: Mann (1945) Econometrica 13:245–259; "
    ...
    if not trend_rows:
        xsc(ws,5,1,"No data available",bg=XC["white"],sz=10)
        return

    hdr = list(trend_rows[0].keys())
    for ci,h in enumerate(hdr,1):
        xsc(ws,5,ci,h,bold=True,fc="FFFFFF",bg=XC["hdr"],border=tb(),sz=9,wrap=True)
    ws.row_dimensions[5].height = 36

    ds_bg = {}
    for ds in set(r["Dataset"] for r in trend_rows):
        if "Observed"  in ds: ds_bg[ds] = XC["obs_r"]
        elif "QDM"     in ds: ds_bg[ds] = XC["bc_r"]
        else:                  ds_bg[ds] = XC["raw_r"]
    ds_fc = {}
    for ds in set(r["Dataset"] for r in trend_rows):
        if "Observed"  in ds: ds_fc[ds] = XC["obs_h"]
        elif "QDM"     in ds: ds_fc[ds] = XC["bc_h"]
        else:                  ds_fc[ds] = XC["raw_h"]

    prev_stn = None
    for ri, rd in enumerate(trend_rows, 6):
        stn = rd.get("Station","")
        ds  = rd.get("Dataset","")
        ...
        for ci,k in enumerate(hdr,1):
            ...
            if ci==2:  # Dataset
                cell.font = Font(bold=True,color=ds_fc.get(ds,"1A1A1A"),
                                 name="Calibri",size=9)
            # Highlight significant trends
            if k=="Significance" and v not in ("—","ns"):
                cell.fill  = xfill(XC["improve"])
                cell.font  = Font(bold=True,color=XC["obs_h"],name="Calibri",size=9)
    ...
    sr = ws.max_row + 2
    mxsc(ws,sr,1,nc,
         "QDM Trend Preservation Summary — Agreement between QDM and Observed trend direction",
         bold=True,fc="FFFFFF",bg=XC["sub"],sz=11)
    ws.row_dimensions[sr].height = 20; sr+=1

    # group by station, compare Observed vs QDM Ensemble direction
    by_stn = {}
    for rd in trend_rows:
        stn = rd["Station"]
        ds  = rd["Dataset"]
        by_stn.setdefault(stn, {})[ds] = rd

    agree_count = 0
    total_count = 0
    sum_hdr = ["Station","Obs Slope","Obs Sig","QDM Slope","QDM Sig",
               "Direction Match","Slope Difference (mm/yr)","Assessment"]
    for ci,h in enumerate(sum_hdr,1):
        xsc(ws,sr,ci,h,bold=True,fc="FFFFFF",bg=XC["hdr"],border=tb(),sz=10,wrap=True)
    ws.row_dimensions[sr].height = 30; sr+=1

    for stn, ds_dict in by_stn.items():
        obs_r = ds_dict.get("Observed")
        qdm_r = ds_dict.get("QDM Ensemble")
        if not obs_r or not qdm_r: continue
        total_count += 1
        obs_dir  = obs_r.get("Trend Direction","")
        qdm_dir  = qdm_r.get("Trend Direction","")
        obs_sl   = obs_r.get("Sen Slope (mm/yr)","—")
        qdm_sl   = qdm_r.get("Sen Slope (mm/yr)","—")
        obs_sig  = obs_r.get("Significance","—")
        qdm_sig  = qdm_r.get("Significance","—")
        match    = (obs_dir == qdm_dir)
        if match: agree_count += 1
        sl_diff = (round(float(qdm_sl)-float(obs_sl),4)
                   if isinstance(obs_sl,(int,float)) and isinstance(qdm_sl,(int,float))
                   else "—")
        assessment = ("✔ Preserved" if match else "✘ Direction Mismatch")
        bg = XC["improve"] if match else XC["degrade"]
        row_vals = [stn, obs_sl, obs_sig, qdm_sl, qdm_sig,
                    "Yes" if match else "No", sl_diff, assessment]
        for ci,v in enumerate(row_vals,1):
            cell = xsc(ws,sr,ci,v,bg=bg,border=tb(),sz=10)
            if ci==8:
                fc_col = XC["obs_h"] if match else XC["raw_h"]
                cell.font=Font(bold=True,color=fc_col,name="Calibri",size=10)
        ws.row_dimensions[sr].height = 16; sr+=1

    # Overall rate
    rate = agree_count/total_count*100 if total_count else 0
    mxsc(ws,sr,1,nc,
         f"Overall QDM Trend Preservation Rate: {agree_count}/{total_count} "
         f"stations ({rate:.1f}%) have consistent trend direction with Observed",
         bold=True, fc="FFFFFF",
         bg="1B5E20" if rate >= 70 else ("E65100" if rate >= 50 else "B71C1C"),
         sz=11)
    ws.row_dimensions[sr].height = 22

    widths = [12,14,20,10,10,10,10,10,8,10,8,20]
    for ci,w in enumerate(widths[:len(hdr)],1): cw(ws,ci,w)
...
def main():
    sep = "=" * 72
    print(sep)
    print("  Multi-Model Ensemble Analysis  v4.0")
    print("  Obs vs Raw CMIP6 (per-model) vs QDM + Regional Average")
    print("  + Ensemble vs Best Model  +  QDM Trend Preservation")
    ...
    # ── NEW E: Ensemble vs Best Single Model ─────────────────────────
    print("\n  วิเคราะห์ Ensemble vs Best Single Model ...")
    ens_vs_best = analyse_ensemble_vs_best(
        model_perf, met_d_raw, met_d_bc, stns_str, province)
    best_mdl = ens_vs_best["best_model"]
    ens_wins = ens_vs_best["ens_wins"]
    print(f"    Best Single Model (QDM): {best_mdl}  "
          f"(Score={ens_vs_best['best_score_qdm']:.4f})")
    print(f"    Ensemble Composite Score: {ens_vs_best['ens_composite']:.4f}")
    print(f"    Ensemble {'OUTPERFORMS' if ens_wins else 'UNDERPERFORMS'} best single model")

    # ── NEW F: QDM Trend Preservation ────────────────────────────────
    print("\n  วิเคราะห์ QDM Trend Preservation (Mann-Kendall) ...")
    trend_rows = analyse_trend_preservation(
        obs_d, raw_ens_d, bc_ens_d,
        model_perf, raw_dfs, bc_dfs,
        stns_str, province)
    print(f"    คำนวณ trend สำหรับ {len(trend_rows)} series")
    ...
    _regional_sheet(wb, reg_perf_d, reg_perf_m, error_red_d, error_red_m,
                    model_df, qdf, rdf, province)
    _ensemble_vs_best_sheet(wb, ens_vs_best, province)
    _trend_preservation_sheet(wb, trend_rows, province)
    ...
    print(sep)
    print(f"  เสร็จสิ้น")
    print(f"  Excel 1 ไฟล์ (9 sheets)  │  รูปภาพ 6 ไฟล์")
