...
def fig_cdf(obs_d, raw_ens, bc_ens, stns, province, out_path):
    ...
    obs_v = pool(obs_d);  raw_v = pool(raw_ens);  bc_v = pool(bc_ens)
    ...
    for panel, (ax, title_tag) in enumerate(zip(axes, tags)):
        ...
        for pct, pcol in [(50, "#78909C"), (90, C["gold"]),
            ...
            if idx >= len(obs_x): idx = len(obs_x) - 1
            xv = obs_x[idx]
            ax.axvline(xv, color=pcol, lw=1.1, ls=":", alpha=0.75)
            ax.text(xv, 0.02 + 0.06*(pct==99) + 0.03*(pct==95),
                    f"P{pct}={xv:.1f}",
                    rotation=90, va="bottom", ha="right",
                    fontsize=FS["note"], color=pcol, fontweight="bold")
        ax.set_xlabel("Daily Rainfall (mm)", fontsize=FS["label"], fontweight="bold")
        ax.set_ylabel("Cumulative Probability", fontsize=FS["label"], fontweight="bold")
        ax.set_title(title_tag, loc="left", fontsize=FS["title"]-2,
                     fontweight="bold", pad=5)
        ax.set_ylim(0, 1)
        ax.tick_params(axis='both', labelsize=FS["tick"], width=1.3)
        ...
        if panel == 1:
            ...
            ax.set_xlim(lo, hi); ax.set_ylim(0.80, 1.0)
...
def main():
    sep = "=" * 72
    print(sep)
    print("  Trend & Distribution Analysis  v3.0  (SMART UPGRADE)")
