"""
================================================================================
  Trend & Distribution Analysis  — Version 3.0  (COMPLETE SMART UPGRADE)
...
================================================================================
  NEW in v3.0:
    ...
    ✓  BUG FIX: column name normalization (int → str) — model data no longer empty
    ✓  BUG FIX: Dry Season correctly spans Nov(Y)–Apr(Y+1) via season_year groupby
    ✓  BUG FIX: Fig I arrows use correct y-coordinates (QDM slope values)
    ✓  Scenario-aware file grouping (historical / ssp245 / ssp585)
    ✓  Full Future SSP Analysis with CMIP6-standard periods:
          Near Future  2021–2040  |  Mid-Century 2041–2060  |  Far Future 2081–2100
    ✓  Fig J — Future Annual Change % vs Historical Baseline (SSP245 + SSP585)
    ✓  Fig K — Future ETCCDI Change Heatmap (3 periods × 2 SSPs)
    ✓  Fig L — Future Annual Rainfall Timeline (2015–2100)
    ✓  Q1 publication-grade fonts: title=18, label=16, tick=14, bold axes
    ✓  Bold axes, heavier lines, DPI=600 throughout

  NEW in v3.2 (Fig D CDF fix — per PDF review + image inspection):
    ✓  FIG D figsize: (18,8) → (18,9) matches Fig E (Q-Q Plot) dimensions
    ✓  FIG D spine linewidth = 1.5  (Q1 standard, consistent with Fig E)
    ✓  FIG D tick: major width=1.5, length=6; minor width=1.0, length=4
    ✓  FIG D minor grid: linestyle=':', alpha=0.25
    ✓  FIG D ROOT CAUSE FIX — percentile label overflow:
          ha='right'+rotation=90 at very small x (P50≈0.1 mm) caused the
          rotated text to extend LEFT off-axes; bbox_inches='tight' then
          captured those objects and rendered them BELOW the saved figure.
          Fix: ha='left' + x-nudge + clip_on=True + staggered y-positions
    ✓  FIG D tail panel (b): skips percentile markers far outside zoom range
    ✓  FIG D ax.set_axisbelow(True) — grid rendered behind data lines
  ─────────────────────────────────────────────────────────────────────────────
...
def fig_pdf(obs_d, raw_ens, bc_ens, stns, province, out_path):
    ...
    ax_ins.set_title("Heavy Tail (> P95)", fontsize=FS["annot"], fontweight="bold")
...
def fig_etccdi_heatmap(obs_d, raw_ens, bc_ens, stns, smap, province, out_path):
    ...
    for ax, mat, title_tag, cmap_ in zip(axes, [mat_obs, mat_raw, mat_bc],
                                          titles, cmaps_):
...
def fig_multiscale_trend_per_model(obs_d, raw_dfs, bc_dfs, bc_ens,
    ...
    for ti, (ts_name, months, ylabel) in enumerate(TIMESCALES):
        ...
        for mi, model in enumerate(models):
            ...
            ax.legend(fontsize=max(7, FS["legend"]-4), frameon=True,
                      edgecolor="#B0BEC5", loc="best", ncol=1)
    ...
    for ax_i, (ax, ylabel, mean_vals, label) in enumerate(zip(
        ...
        if ax_i < 2:
            ...
            for bar, v in zip(bars, mean_vals):
                ax.text(bar.get_x()+bar.get_width()/2, v+abs(v)*0.01,
        ...
        ax.set_xticks(range(len(keys_plot)))
...
def main():
    sep = "=" * 72
    print(sep)
    print("  Trend & Distribution Analysis  v3.2  (CDF FIX + Q1 Consistency)")
