...
"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  Ensemble Saturation Curve Analysis — v3.0                                  ║
║  "How Many CMIP6 Models Are Enough for Station-Scale Rainfall?"             ║
║  Q2–Q3 Publication Standard  |  Academically Rigorous                       ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  NEW IN v3.0 (vs v2.0):                                                      ║
║  [1] PARALLEL PROCESSING — concurrent.futures.ProcessPoolExecutor           ║
║      distributes C(M,N) combos across all CPU cores                         ║
║  [2] UNIT TESTING — pytest-compatible test suite (test_saturation_v3.py)   ║
║      validates KGE, NSE, RMSE formulas against known solutions              ║
║  [3] DATA VALIDATION — pre-run integrity checks:                            ║
║      date range overlap, missing-value %, station consistency               ║
║  [4] BOOTSTRAP CI FOR SATURATION POINT (N*) — 95% CI on N* via             ║
║      resampling of C(M,N) subset performance values                         ║
║  [5] ENHANCED EXCEL — S4 sheet: Bootstrap CI + statistical summary         ║
║  [6] IMPROVED FIGURES — no suptitle, full-size, bold publication colors,    ║
║      non-overlapping legends, consistent Q2–Q3 standard across all 3 figs  ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Figures:                                                                    ║
║   Fig 1 – Ensemble Performance Saturation (4 metrics, ±1 SD, N* ★)        ║
...
import os, sys, re, math, warnings, gc, itertools, time
import numpy as np
import pandas as pd
...
# ════════════════════════════════════════════════════════════════════════
#  §12  EXCEL OUTPUT — 4 SHEETS  [Sheet 4 = Bootstrap CI, NEW in v3.0]
# ════════════════════════════════════════════════════════════════════════
...
def main():
    ...
    print("  Q2–Q3 Publication Standard  |  3 Figs + Excel (4 sheets) + Word + Tests")
    print(SEP)
