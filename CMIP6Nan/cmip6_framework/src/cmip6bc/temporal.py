...
def temporal_metrics(series: pd.Series, wet_thr: float = 1.0) -> dict:
    ...
    A missing day is a break in the sequence, not something to be closed up:
    dropping it would join the days either side and inflate spell lengths and
    transition counts. Missing days are therefore kept in place and excluded
    from every pair and run, while amount-based statistics use the finite
    values only.
    """
    ...
    if int(ok.sum()) < 30:
        return {}
    # wet, dry and missing are three states; a run is broken by a missing day
    wet = np.where(ok, x >= wet_thr, False)
    dry = np.where(ok, x < wet_thr, False)
    ...
    # only consecutive pairs in which both days were observed can be counted
    pair = ok[:-1] & ok[1:]
    ww = wet[:-1] & wet[1:] & pair
    dw = dry[:-1] & wet[1:] & pair
    n_w, n_d = int((wet[:-1] & pair).sum()), int((dry[:-1] & pair).sum())

    wet_spells = _spells(wet)
    dry_spells = _spells(dry)

    # amount-based statistics use the observed days; the autocorrelation of
    # occurrence is computed on the observed pairs so a gap is not bridged
    xo = x[ok]
    occ = wet.astype(float)
    out = {
        "acf1_daily": _acf1(xo),
        "acf1_occurrence": (float(np.corrcoef(occ[:-1][pair], occ[1:][pair])[0, 1])
                            if pair.sum() > 2
                            and occ[:-1][pair].std(ddof=0) > 0
                            and occ[1:][pair].std(ddof=0) > 0 else np.nan),
