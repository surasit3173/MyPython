...
def apply(f: Fit, target, ) -> tuple[np.ndarray, dict]:
    ...
    if wet.any():
        ...
        if f.method == "qm":
            ...
            # Tied daily totals are common once rainfall is recorded to 0.1 mm.
            # Interpolating on a non-strictly-increasing sample would give every
            # member of a tied group the probability of the last member; the
            # average (mid) rank is used instead, so tied values receive one
            # shared probability. The rule is stated in the method settings.
            p_t = _plotting_probability(w, f.pp_kind)
...
def _plotting_probability(x: np.ndarray, kind: str) -> np.ndarray:
    """Empirical non-exceedance probability with average ranks for ties."""
    from scipy.stats import rankdata
    r = rankdata(x, method="average")
    n = len(x)
    a = {"hazen": 0.5, "weibull": 0.0, "gringorten": 0.44}.get(kind, 0.5)
    return (r - a) / (n + 1 - 2 * a)
