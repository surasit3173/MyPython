...
DRAFT = WORK / "revision_round2" / "manuscript_revised.md"
AUDIT = WORK / "audit_corrected"
OUTPUT_DIR = WORK / "revision_round2" / "output"
OUTPUT = OUTPUT_DIR / "APST_Uttaradit_CMIP6_Revised_Q2Q3.docx"
REPORT = OUTPUT_DIR / "APST_Uttaradit_CMIP6_Revised_Q2Q3.build.json"

FIGURES = {
    1: WORK / "future_zip" / "output" / "future_q1_uttaradit" / "figures" / "FUTURE_Q1_FIGURE_01_study_area.png",
    2: WORK / "revision_round2" / "figures" / "FIGURE_02_baseline_sensitivity_revised.png",
    3: AUDIT / "figures" / "FIGURE_03_change_heatmap.png",
    4: WORK / "revision_round2" / "figures" / "FIGURE_04_selected_profiles_revised.png",
}
...
FIGURE_META = {
    ...
    2: (
        5.70,
        "Sensitivity of regional median relative changes to baseline source. The x-axis uses observations for 1995–2014; the y-axis uses each model's supplied bias-corrected historical series for 1995–2014. Colors and symbols distinguish projection periods. Each point is an index–period combination; R50mm is excluded because percentage changes are unstable at small baselines. Crosses mark sign reversals and the dashed line is 1:1. Results are descriptive rather than significance tests.",
        "Scatter plot comparing future-change estimates from observed and model-consistent baseline sources.",
    ),
    3: (
        5.70,
        "Audited multimodel regional median relative changes from the model-consistent baseline. Values are percentages; an asterisk indicates that at least six of seven models agreed on direction. R50mm percentages are shown only for completeness and are denominator-sensitive. The full model-consistent and observed-reference comparison is in Supplementary Table S1.",
        "Heatmap of audited precipitation-index changes by scenario and future assessment period.",
    ),
    4: (
        5.70,
        "Selected model-consistent projection profiles. Points show seven-model medians and bands show interquartile ranges. Filled markers indicate agreement by at least six of seven models; open markers are ambiguous. Colors distinguish scenarios.",
...
def set_run_font(run, size: float = 10.0, bold=None, italic=None) -> None:
    ...
    if italic is not None:
        run.italic = italic
    if re.search(r"\[(?:Author|Department|Corresponding|Before submission|email)", run.text, flags=re.IGNORECASE):
...
def build_table_2(document: Document) -> None:
    ...
    add_table_caption(document, 2, "Median historical performance across 91 model–gauge pairs for raw and supplied bias-corrected series.")
    add_native_table(
        document,
...
def build_table_3(document: Document) -> None:
    ...
    p = document.add_paragraph()
    set_paragraph_format(p, alignment=WD_ALIGN_PARAGRAPH.LEFT, line_spacing=1.0)
    add_markdown_runs(p, "Note: IQR is the interquartile range across seven model-level regional changes. Six of seven models (85.7%) is the first integer count satisfying the prespecified ≥80% sign-agreement criterion; this is descriptive, not a significance test. R50mm is reported in signed native units.", 10)
