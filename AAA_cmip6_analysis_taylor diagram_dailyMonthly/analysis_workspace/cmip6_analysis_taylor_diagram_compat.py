"""
╔══════════════════════════════════════════════════════════════════════════════╗
║  CMIP6 Bias Correction — Comprehensive Analysis for Q2–Q3 Publication      ║
║  Focus: Taylor Diagram · QQ Plot · Statistical Metrics · Significance      ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  Figures generated:                                                          ║
║   Fig 1  Taylor Diagram — Daily (all models + ensemble per station)         ║
║   Fig 2  Taylor Diagram — Monthly (all models + ensemble per station)       ║
║   Fig 3  Q-Q Plot — Daily (Raw vs BC per model, pooled stations)            ║
║   Fig 4  Q-Q Plot — Monthly (Raw vs BC per model, pooled stations)          ║
║   Fig 5  Performance Heatmap — Daily metrics all stations × all models      ║
║   Fig 6  Performance Heatmap — Monthly metrics all stations × all models    ║
║   Fig 7  Metric Improvement Bar (RMSE, Pbias, r, NSE, KGE, d) Daily+Mo     ║
║   Fig 8  Wilcoxon Signed-Rank & KS-test significance per station            ║
║   Fig 9  Monthly Climatology — Obs vs Raw vs BC ensemble                    ║
║   Fig 10 Annual Time Series — Regional mean with ensemble spread            ║
║                                                                              ║
║  Excel output (8 sheets):                                                   ║
║   S1  Statistical Metrics — Daily                                           ║
║   S2  Statistical Metrics — Monthly                                         ║
║   S3  Improvement Summary (Raw → BC, % change)                             ║
║   S4  Wilcoxon Signed-Rank Test Results                                     ║
║   S5  KS-Test Results                                                       ║
║   S6  Seasonal Analysis (Wet/Dry season)                                    ║
║   S7  Journal-ready Summary (ready to paste in paper)                      ║
║   S8  Methods & References                                                  ║
║                                                                              ║
║  Input (auto-discovered from folder):                                       ║
║    Observed : *Observed*.csv                                                ║
║    Raw      : pr_<MODEL>_*.csv                                              ║
║    BC/QDM   : bc_<MODEL>_*.csv                                              ║
╠══════════════════════════════════════════════════════════════════════════════╣
║  References:                                                                 ║
...
║    Willmott (1981) Phys.Geogr. 2:184–194               [IoA / d]          ║
║    Cannon et al. (2015) J.Climate 28:6938–6959         [QDM]               ║
║    Moriasi et al. (2007) Trans.ASABE 50:885–900        [Performance Crit.] ║
║    Wilcoxon (1945) Biometrics Bull. 1:80–83            [Wilcoxon test]     ║
║    Massey (1951) J.Am.Stat.Assoc. 46:68–78             [KS test]           ║
║    Benjamini & Hochberg (1995) J.R.Stat.Soc.B 57:289  [FDR correction]    ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
...
RMSE_RS_GOOD  = 0.60   # RSR = RMSE/σ_obs < 0.60 → Good
...
def fdr_correction(pvals,alpha=0.05):
    ...
    n=len(pvals)
    if n==0: return []
    idx=sorted(range(n),key=lambda i:pvals[i] if not np.isnan(pvals[i]) else 1.1)
    reject=[False]*n
    for rank,(i) in enumerate(idx,1):
        if not np.isnan(pvals[i]) and pvals[i]<=alpha*rank/n:
            reject[i]=True
    return reject
...
def _taylor_bg(ax,ref_std,unit):
    ...
    ax.set_xlabel(f"Standard Deviation ({unit})",fontsize=12,fontweight="bold",labelpad=5)
    ax.set_ylabel(f"Standard Deviation ({unit})",fontsize=12,fontweight="bold",labelpad=5)
    ...
    for si,stn in enumerate(stns_str):
        ...
        if show_individual:
            for m,raw_df in raw_dfs.items():
                ...
                ss=so*sr if not(np.isnan(so) or np.isnan(sr)) else np.nan
...
def _draw_panel_a(ax, obs_df, raw_dfs, bc_dfs, stns_str, smap,
    ...
    for si, stn in enumerate(stns_str):
        ...
        for ens_df, mk, ds, zord in [
            ...
            so2 = mr2.get("std_obs", np.nan)
            ss2 = so2 * sr2 if not (np.isnan(so2) or np.isnan(sr2)) else np.nan
...
def build_taylor_fig(obs_df, raw_dfs, bc_dfs, stns, smap,
    ...
    # Compatibility with Matplotlib releases where matplotlib.cm.get_cmap
    # is no longer exposed; pyplot.get_cmap preserves the same behavior.
    cmap_s      = plt.get_cmap("tab20", max(len(stns), 1))
    ...
    # Reference std: regional mean across stations (observed)
    all_stds = [float(obs_df[s].std(ddof=1))
                for s in stns_str
                if s in obs_df.columns and obs_df[s].notna().sum() > 5]
    ref_std = float(np.nanmean(all_stds)) if all_stds else 5.0

    sup_title = (
        f"Taylor Diagram — {scale_label} Scale\n"
        f"CMIP6 Raw (▲) vs Bias-Corrected QDM (●)  |  "
    ...
    _draw_panel_a(ax_a2, obs_df, raw_dfs, bc_dfs, stns_str, smap,
                  stn_colors, ref_std, unit, scale_label)
    fig_a.suptitle(
        f"Taylor Diagram — {scale_label} Scale  (a) Station Colours\n"
    ...
    _draw_panel_b(ax_b2, obs_df, raw_dfs, bc_dfs, stns_str, smap,
                  model_list, mc, ref_std, unit, scale_label)
    fig_b.suptitle(
        f"Taylor Diagram — {scale_label} Scale  (b) Model Colours\n"
        f"Obs: {period_obs}  |  Ref: Taylor (2001)",
        fontsize=14, fontweight="bold"
    )
...
def build_qq_fig(obs_df,raw_dfs,bc_dfs,stns,
    ...
    4 panels: per model (Raw vs Obs) + ensemble BC.
    Colour-coded by quantile level + extreme markers.
    """
    stns_str=[str(s) for s in stns]
    model_list=sorted(set(list(raw_dfs.keys())+list(bc_dfs.keys())))
    ...
    def _qq_panel(ax,sim_v,col,title,panel_tag):
        if len(sim_v)<10:
            ax.text(0.5,0.5,"Insufficient data",transform=ax.transAxes,
                    ha="center",fontsize=11)
            ax.set_title(f"{panel_tag}  {title}",loc="left",
                         fontsize=12,fontweight="bold"); return np.nan,np.nan
        q_sim=np.percentile(sim_v,probs)
        xy_max=max(q_obs.max(),q_sim.max())*1.04
        # 1:1 line
        ax.plot([0,xy_max],[0,xy_max],color=C["green"],lw=1.6,ls="--",
                alpha=0.85,label="1:1 (perfect)",zorder=2)
        # Scatter
        sc=ax.scatter(q_obs,q_sim,c=probs,cmap="RdYlBu_r",
                      s=18,alpha=0.72,edgecolors="none",zorder=3)
        # Extreme markers
        for pct,pc,lbl,mk in pct_marks:
            qo=float(np.percentile(obs_v,pct))
            qs=float(np.percentile(sim_v,pct))
            ax.scatter([qo],[qs],color=pc,s=90,zorder=6,
                       marker=mk,edgecolors="white",linewidths=0.7,
                       label=f"{lbl}: Obs={qo:.1f}, Sim={qs:.1f}")
        # QQ-RMSE annotation
        qq_rmse=float(np.sqrt(np.mean((q_sim-q_obs)**2)))
        r2=float(np.corrcoef(q_obs,q_sim)[0,1]**2)
        ax.text(0.04,0.96,
                f"QQ-RMSE={qq_rmse:.2f} mm\nr²={r2:.4f}",
                transform=ax.transAxes,fontsize=9.5,fontweight="bold",
                va="top",ha="left",
                bbox=dict(boxstyle="round,pad=0.3",fc="white",
                          ec="#B0BEC5",alpha=0.88))
        ...
        cb.set_label("Percentile (%)",fontsize=9,fontweight="bold")
        cb.ax.tick_params(labelsize=8)
        return qq_rmse,r2

    panel_idx=0
    # Individual models (Raw)
    for mi,(m,col) in enumerate(zip(model_list,mc)):
        raw_df=raw_dfs.get(m)
        bc_df =bc_dfs.get(m)
        raw_v=pool_wet(raw_df)
        bc_v =pool_wet(bc_df)
        ax=axes[panel_idx]
        # Split into left/right sub-axes manually via inset
        _qq_panel(ax,raw_v,col,f"{m} — Raw vs Obs",chr(97+panel_idx))
        panel_idx+=1

    # Ensemble BC panel
    ax_ens=axes[panel_idx]
    bc_ens_v=pool_wet(bc_ens)
    _qq_panel(ax_ens,bc_ens_v,C["ens"],"Ensemble BC (QDM) vs Obs",chr(97+panel_idx))
    panel_idx+=1
...
def build_improvement_fig(obs_d,obs_m,raw_dfs,bc_dfs,
    ...
    """
    Fig 7: Metric Improvement (Daily + Monthly) in 2×3 sub-panels.
    Shows delta(BC−Raw) for RMSE, |Pbias|, r, NSE, KGE, d.
    """
    stns_str=[str(s) for s in stns]
    codes=[smap[s] for s in stns_str]
    ...
    def deltas(obs_df,raw_ens,bc_ens,met,invert=False):
        out=[]
        for stn in stns_str:
            ...
            elif invert: out.append(vr-vb)   # lower-better: Raw−BC
            else:        out.append(vb-vr)   # higher-better: BC−Raw
        return out

    METS=[("RMSE","RMSE Reduction\n(mm)",True),
          ("Pbias_abs","|Pbias| Reduction\n(%)",True),
          ("r","r Improvement",False),
          ("NSE","NSE Improvement",False),
          ("KGE","KGE Improvement",False),
          ("d","d (IoA) Improvement",False)]
    ...
    for pi,(met,ylabel,inv) in enumerate(METS):
        ax=axes[pi//3,pi%3]
        d_d=deltas(obs_d,raw_ens_d,bc_ens_d,met,inv)
        d_m=deltas(obs_m,raw_ens_m,bc_ens_m,met,inv) if obs_m is not None else [np.nan]*len(stns)
        b1=ax.bar(x-bw/2,d_d,width=bw,color=C["bc_lt"],
                  edgecolor=C["bc_bd"],linewidth=0.9,alpha=0.88,
                  label="Daily",zorder=3)
        b2=ax.bar(x+bw/2,d_m,width=bw,color=C["ens_lt"],
                  edgecolor=C["ens_bd"],linewidth=0.9,alpha=0.88,
        ...
        tag=chr(97+pi)
        ax.set_title(f"({tag})  {ylabel.split(chr(10))[0]}",
                     loc="left",fontsize=12,fontweight="bold",pad=4)
        ax.yaxis.set_minor_locator(ticker.AutoMinorLocator())
        ax.spines["top"].set_visible(False); ax.spines["right"].set_visible(False)
        if pi==0:
            ax.legend(fontsize=10.5,frameon=True,edgecolor="#B0BEC5",
                      facecolor="white",framealpha=0.92,loc="upper right",ncol=2)

    fig.suptitle(
        "Improvement in Statistical Metrics After QDM Bias Correction\n"
        "Blue = Daily | Green = Monthly | Red bar = degradation",
        fontsize=14,fontweight="bold"
    )
...
def build_significance_fig(wilcox_rows,ks_rows,stns,smap,period_obs,out_dir,prefix):
    ...
    ax.set_xticks(x); ax.set_xticklabels(codes,rotation=0,fontsize=10)
...
def build_monthly_climatology(obs_d,raw_dfs,bc_dfs,stns,smap,
    ...
    def monthly_clim(df,cols):
        ...
        reg=sub.mean(axis=1)
        clim=reg.groupby(reg.index.month).agg(["mean","std"])
    ...
    ax0.set_title("Regional Mean Monthly Climatology (1±σ shading)",
                  loc="left",fontsize=13,fontweight="bold",pad=5)
    ...
    for si,stn in enumerate(stns_str):
        ...
        def stn_clim(df):
            ...
            c=df[stn].groupby(df[stn].index.month).agg(["mean","std"])
...
def write_excel(wb,obs_d,obs_m,raw_dfs,bc_dfs,raw_m_dfs,bc_m_dfs,
    ...
    def _perf_sheet(ws_name,obs_df,raw_ens,bc_ens,scale):
        ...
        _title_row(ws,nc,
            ...
            "★=best Raw vs BC  |  Green=improved  |  Ref: Gupta et al.(2009); Nash & Sutcliffe(1970)")
        ...
        for ds_lbl,ens_df,bg_key in [("Raw CMIP6",raw_ens,"raw_r"),
            ...
            for stn,code in zip(stns_str,codes):
                ...
                for ci,v in enumerate(vals,1):
                    is_best=(stn,MET_KEYS[ci-5]) in best_set if ci>=5 else False
                    bg2=XC["best"] if is_best else (
                        XC["improve"] if (ci>=5 and ds_lbl=="BC (QDM)" and
                            (stns_str[0],MET_KEYS[ci-5]) in best_set) else bg)
    ...
    _title_row(ws3,nc3,
        ...
        "Δ = BC minus Raw (positive = improved for higher-is-better metrics; "
        "positive = reduced for lower-is-better metrics)  |  "
        "% = relative change  |  Green=improved  Red=degraded")
    hdrs3=["Station","Code",
           "ΔRMSE (mm)","ΔRMSE (%)","ΔPbias","Δr","ΔNSE","ΔKGE","Δd",
    ...
    for scale,obs_df,raw_e,bc_e in [("Daily",obs_d,raw_ens_d,bc_ens_d),
        ...
        for stn,code in zip(stns_str,codes):
            ...
            for ci,v in enumerate(row,1):
                bg=XC["white"]
                if ci in (3,4) and not np.isnan(drmse): bg=XC["improve"] if drmse>0 else XC["degrade"]
                if ci==8 and not np.isnan(dkge):        bg=XC["improve"] if dkge>0  else XC["degrade"]
    ...
    hdrs6=["Season","Station","Code","KGE Raw","KGE BC","ΔKGE",
           "RMSE Raw (mm)","RMSE BC (mm)","ΔRMSE (mm)","Interpretation"]
    ...
    for season,obs_sea,raw_sea,bc_sea in [
        ...
        for stn,code in zip(stns_str,codes):
            ...
            row=[season,stn,code,
                 _fmt(kr),_fmt(kb),
                 f"{_fmt(dkge)}{'↑' if not np.isnan(dkge) and dkge>0 else '↓' if not np.isnan(dkge) else ''}",
                 _fmt(rr,3),_fmt(rb,3),
    ...
    sections=[
        ...
        ("METHODS — STATISTICAL TESTS",
         ...
         "(H₁: |err_BC| < |err_Raw|, one-tailed, α=0.05), with False Discovery Rate (FDR) "
    ...
    refs=[
        ("Taylor Diagram",
         ...
         "Visualization of r, σ_r, RMSE simultaneously"),
        ("KGE",
        ...
        ("NSE",
         ...
         "NSE=1−Σ(S−O)²/Σ(O−Ō)²; VG>0.75, Good 0.65–0.75, Sat 0.50–0.65"),
        ("IoA / d",
        ...
        ("RSR",
         ...
         "ISWS CR 2004-08.",
         "RSR=RMSE/σ_obs; VG<0.50, Good<0.60, Satisfactory<0.70"),
    ]
...
def main():
    ...
    print("  มาตรฐาน Q2–Q3 / Nature / Elsevier")
    print(SEP)
    ...
    print(f"  Excel      : {out_xl.name}  (8 sheets)")
    print(f"  {'─'*62}")
    print(f"  Regional mean KGE — Raw: {kge_r_m:.3f}  →  BC: {kge_b_m:.3f}  "
          f"(Δ{kge_b_m-kge_r_m:+.3f})")
