...
def wilcoxon_paired(a: np.ndarray, b: np.ndarray, lower_better: bool = True) -> dict:
    ...
    รายงาน median difference, n_improved และ matched-pairs rank-biserial (effect size)
    ข้อจำกัด: 12 สถานีไม่เป็นอิสระเชิงพื้นที่ ค่า p จึงเป็น anti-conservative
    """
    ...
    if n < 5:
        return {"n": n}
    diff = b - a
    imp = int((diff < 0).sum()) if lower_better else int((diff > 0).sum())
    nz = diff[diff != 0]
    if nz.size < 3:
        return {"n": n, "n_improved": imp, "median_diff": float(np.median(diff))}
...
def table_paired(classic: pd.DataFrame, scale="monthly", period="validation",
    ...
    if not df.empty and "p_value" in df:
        df["fdr_pass"] = benjamini_hochberg(df["p_value"].fillna(1).values, cfg.fdr_q)
    return df
...
def table4_change(classic, paired, scale="monthly") -> pd.DataFrame:
    ...
    if not paired.empty:
        for met in ("RMSE", "PBIAS", "NSE"):
            ...
            out[f"{met}_n_improved"] = p.get("n_improved")
            out[f"{met}_p"] = p.get("p_value").round(4) if "p_value" in p else np.nan
            out[f"{met}_FDR"] = p.get("fdr_pass")
    return out
...
def cmd_inspect(folder: str, cfg: Config) -> None:
    ...
    for mdl in sorted(data["raw"]):
        sub = fp[fp.model == mdl]
        for cell, g in sub.groupby("grid_cell_id"):
            print(f"    {cell}: {', '.join(g.station)}")
...
def cmd_run(folder: str, out: str, cfg: Config,
            homogeneous_only: bool = False, exclude: list[str] | None = None) -> None:
    outdir = Path(out)
    ...
    if homogeneous_only:
        drop += [s for s in qc_full.index[qc_full["flag_inhomogeneous"] |
                                          qc_full["flag_smoothed"]] if s not in drop]
    if drop:
