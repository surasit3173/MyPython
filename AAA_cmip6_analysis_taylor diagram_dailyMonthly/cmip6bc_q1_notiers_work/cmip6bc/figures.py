...
C_CAL, C_VAL = "#0072B2", "#D55E00"
C_RAW, C_QDM = "#C0392B", "#1F5FA9"
C_REF, C_GRID = "#2F2F2F", "#B0B0B0"
C_GOOD, C_BAD, C_NEUT = "#2E7D32", "#B22222", "#808080"
...
def apply_style(fig_cfg: dict) -> None:
    ...
    plt.rcParams.update({
        ...
        "grid.alpha": 0.55, "grid.color": C_GRID,
...
def ptitle(ax, tag, text="", pad=5):
    ...
    ax.set_title(f"({tag}) " + (text if text else ""), loc="left",
                 fontsize=FS * 0.88, fontweight="semibold", pad=pad)
...
def figure2(cfg, d: Path):
    ...
    fig = plt.figure(figsize=(W, 0.50 * W), layout="constrained")
    ...
    def box(x, y, w, h, title, lines, fc, ec):
        ...
        ax.text(x + w / 2, y + h - 3.2, title, ha="center", va="top",
                fontsize=FS * 0.86, fontweight="bold", color="#1A1A1A", zorder=3)
        for k, ln in enumerate(lines):
            ax.text(x + w / 2, y + h - 9.0 - k * 5.2, ln, ha="center", va="top",
                    fontsize=FS * 0.76, color="#2A2A2A", zorder=3)
    ...
    box(23, 6, 54, 15, "Performance assessment",
        ["four metric groups \u00B7 year-block bootstrap"], GREY_F, GREY_E)
...
def figure3(obs, cfg, d: Path):
    ...
    fig = newfig(0.345, wsp=0.09)
    gs = fig.add_gridspec(1, 3, width_ratios=[1.00, 1.06, 0.86])
    ...
    ax.set_ylim(0, 2050); ax.set_xlim(1979.2, 2015.8)
    ax.set_xticks([1980, 1990, 2000, 2010])
    ax.text(1990.5, 1930, "Calibration", color=C_CAL, ha="center", va="top",
            fontsize=FS * 0.78, fontweight="bold")
    ax.text(2008.0, 1930, "Validation", color=C_VAL, ha="center", va="top",
            fontsize=FS * 0.78, fontweight="bold")
    ...
    ax.plot([lo, hi], [lo, hi], color=C_REF, lw=1.0, ls=(0, (5, 3)), zorder=2,
            label="1:1")
    ax.scatter(list(wf_c.values()), list(wf_v.values()), s=38, marker="o",
               facecolor=C_QDM, edgecolor="white", linewidth=0.6, alpha=0.95,
               zorder=3)
    ax.set_xlim(lo, hi); ax.set_ylim(lo, hi); ax.set_aspect("equal")
...
def figure4(metrics, d: Path):
    ...
    fig = newfig(0.74, wsp=0.10, hsp=0.14)
...
def figure5(metrics, d: Path):
    ...
    for k, (col, lab, ab) in enumerate(panels):
        ...
        cb = fig.colorbar(im, ax=ax, fraction=0.026, pad=0.014, aspect=8)
...
def figure6(metrics, mme, d: Path):
    ...
    for k, dsname in enumerate(("raw", "QDM")):
        ...
        ax = fig.add_subplot(gs[k, 0])
        bp = ax.boxplot(data, widths=0.52, patch_artist=True, showfliers=True,
                        ...
                        flierprops=dict(marker="o", markersize=3.0,
                                        ...
                                        markeredgewidth=0.6))
        for p in bp["boxes"]:
            p.set(facecolor="#D6E4F2", edgecolor="#41627F", linewidth=0.8)
...
def figure7(metrics, d: Path):
    ...
    fig = newfig(0.44, wsp=0.11)
    gs = fig.add_gridspec(1, 3, width_ratios=[1.0, 1.0, 0.94])
