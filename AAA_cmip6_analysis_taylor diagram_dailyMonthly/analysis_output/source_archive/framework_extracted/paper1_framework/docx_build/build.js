...
const CAP = (t, o = {}) => new Paragraph({
  ...
  spacing: { line: 240, before: o.before === undefined ? 60 : o.before, after: o.after === undefined ? 100 : o.after },
  keepNext: !!o.keepNext,
});
...
function figure(file, widthIn) {
  ...
  return new Paragraph({
    ...
    spacing: { before: 120, after: 40, lineRule: d.LineRuleType.AUTO },
...
body.push(BLANK());
body.push(P("**Keywords:** Bias correction, CMIP6, Extreme precipitation, Near-future projection, Out-of-sample validation, Quantile delta mapping, Thailand"));
body.push(BLANK());

// Body with tables and figures anchored after the subsection they support
const AFTER_SH = {
  "2.1 Study area and rain-gauge data": () => [
    figure("Paper1_Figure_01_study_area.png", 5.4),
    ...
    CAP("**Table 1** Rain gauges used in this study: identifier, coordinates, record period and mean annual wet-day rainfall over 1981–2002. Wet-day frequency, SDII, quality-control flags and the homogeneity screening are given in Supplementary Table S1.", { keepNext: true, after: 60 }),
    makeTable(["Station", "Longitude (°E)", "Latitude (°N)", "Record", "Mean annual rainfall (mm)"],
      J("T1.json").map((r) => [String(r.Station), r["Longitude (°E)"].toFixed(2),
        r["Latitude (°N)"].toFixed(2), r.Record, String(Math.round(r["Mean annual rainfall (mm)"]))]),
      [1.4, 1.8, 1.6, 1.6, 2.4], [L, R, R, C, R]),
  ],
  "3.1 Calibration-period performance": () => [
    ...
    biasTable(J("T2.json")),
    figure("Paper1_Figure_02_calibration.png", 5.8),
  ...
  "3.2 Independent validation": () => [
    ...
    biasTable(J("T3.json")),
    figure("Paper1_Figure_03_validation.png", 5.8),
  ...
  "3.3 Near-future precipitation changes, 2021\u20132050": () => [
    ...
    figure("Paper1_Figure_04_near_future_precipitation.png", 6.4),
  ...
  "3.4 Near-future rainfall extremes": () => [
    figure("Paper1_Figure_05_near_future_extremes.png", 6.4),
  ...
  "3.5 Gauge-level spatial pattern and model agreement": () => [
    figure("Paper1_Figure_06_near_future_map.png", 6.6),
    CAP("**Figure 6** Ensemble median change in annual precipitation for 2021\u20132050 relative to each model's own bias-corrected baseline. The shaded surface is an inverse-distance-weighted interpolation between the 13 gauges, clipped to the verified provincial boundary; the gauges themselves are drawn on top, sized by the inter-model agreement fraction, with filled symbols for agreement of at least 0.80 and open symbols below it."),
  ],
};
...
for (let i = 0; i < secs.length; i++) {
  ...
  if (s.t === "h") {
    ...
    body.push(P(s.v, { indent: !refMode && !s.v.startsWith("**Keywords") }));
