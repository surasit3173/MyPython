...
def is_path_traversal(scope: str) -> bool:
    """
    Check if scope contains path traversal sequences, absolute path roots, or path escape vectors.
    """
    ...
    p = Path(s)
    if p.is_absolute() or s.startswith("/") or s.startswith("\\") or re.match(r"^[a-zA-Z]:", s):
        return True

    # Reject path traversal or relative root components
    parts = p.parts
    if ".." in parts or "." in parts:
        return True

    # Also check string split to catch windows/posix slash variations
    split_parts = re.split(r"[/\\]", s)
    if ".." in split_parts or "." in split_parts:
        return True
