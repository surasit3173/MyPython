...
- `research_controller/github_client.py`: Secure GitHub REST API client with secret redaction, dry-run mode, retry logic, timeout handling, and authorization gates.
...
- `research_controller/tests/test_github_jules_integration.py`: Unit and mock test suite covering all security and failure modes.
...
- `research_controller/controller.py` (MODIFIED: added cross-platform `RESEARCH_BASE_DIR` support)
...
Unit and integration test suite executed via `PYTHONPATH=research_controller python3 -m unittest discover -s research_controller/tests`:
