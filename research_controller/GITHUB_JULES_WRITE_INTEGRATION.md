...
- **Authorization Enforcement**: Every write call and internal `_request()` call verifies that `authorization="EXPLICIT_HUMAN_AUTHORIZED"`.
- **Operation Allowlist**: Only operations present in `ALLOWED_WRITE_OPERATIONS` (`CREATE_ISSUE`, `ADD_ISSUE_COMMENT`, `ADD_PR_COMMENT`, `GET_ISSUE`, `GET_PR`, `GET_COMMIT`) are permitted.
- **Forbidden Action Guard**: Operations in `FORBIDDEN_AUTOMATED_OPERATIONS` are strictly rejected.
...
- `GET_PR`: Inspect current status of PRs.
- `GET_ISSUE`: Inspect current status of issues.
- `GET_COMMIT`: Inspect commit metadata.
...
- Validates operation against allowlist and forbidden list.
