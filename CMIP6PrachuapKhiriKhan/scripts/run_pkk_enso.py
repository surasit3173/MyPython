"""Execute the frozen Paper 3 ENSO/rainfall analysis end to end for Prachuap Khiri Khan."""
...
PAPER3_SRC = REPO_ROOT / "AAA_cmip6_analysis_taylor diagram_dailyMonthly" / "paper3_execution" / "src"
if str(PAPER3_SRC) not in sys.path:
    sys.path.insert(0, str(PAPER3_SRC))
...
def prepare_enso(
    ...
    return observed_classes, model_classes_frame, model_catalog_frame
...
def _annotate_metrics(
    ...
    *,
    ...
    output["model"] = model
    output["grid_signature"] = output["station"].map(signatures)
    output = output.merge(
        classification[
            ["climate_year", "season_type", "enso_phase", "index_source", "season_id"]
        ],
        on=["climate_year", "season_type"],
        how="left",
...
def rainfall_pipeline(
    ...
    if len(stations) != config["study"]["station_count_expected"]:
        raise RuntimeError(f"expected {config['study']['station_count_expected']} observed gauges, found {len(stations)}")
    ...
    qdm_diag_frame = pd.concat(qdm_diagnostics, ignore_index=True)
    qdm_diag_frame.to_csv(output / "crossfit_qdm_diagnostics.csv", index=False)
    pd.DataFrame(signature_rows).to_csv(
        output / "raw_grid_signatures.csv", index=False
    )

    return metrics_frame, qdm_diag_frame, qc_summary, provenance
...
def response_analysis(
    ...
    *,
    ...
    metric_names = config["metrics"]["primary"]
...
def build_audits(
    ...
    *,
    ...
    expected_stations = config["study"]["station_count_expected"]
    expected_models = 5
    ...
    gates = [
        ...
        {
            "gate": "P3-D",
            "description": f"Common period contains {expected_stations} gauges and {expected_models} models",
            "pass": bool(metrics["station"].nunique() == expected_stations and metrics.loc[metrics["source_type"] == "RAW", "model"].nunique() == expected_models),
        },
        {
            "gate": "P3-E",
    ...
    manifest = {
        "config": str(Path(config["paths"]["observed"]).name),
...
def main() -> None:
    ...
    parser.add_argument("--config", default="CMIP6PrachuapKhiriKhan/config_pkk_enso.yaml")
    ...
    print(f"PRACHUAP_ENSO_ANALYSIS_COMPLETE mode={'quick' if args.quick else 'full'} output={output}")
