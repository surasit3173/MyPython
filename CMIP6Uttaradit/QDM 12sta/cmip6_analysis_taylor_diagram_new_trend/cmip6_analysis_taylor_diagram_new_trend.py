...
VERSION    = "3.6-Q1-ETCCDI-extremes-refined"
...
def _effective_n(values, lag_k=TREND_LAG_K):
    ...
    for k in range(1, min(lag_k,n-2)+1):
        ...
        if np.isnan(rho):
            continue
        # Bartlett-style effective sample size adjustment.
        denom += 2.0*(1.0-k/n)*rho
        rhos.append((k,rho))
    if denom<=0:
        return n, rhos
    neff=max(3.0, min(float(n), n/denom))
    return neff, rhos
...
def append_etccdi_excel(wb, thresholds, annual_rows, summary_rows, bias_rows, trend_rows):
    ...
    _write_table(
        ...
        "R95p and R99p use station-specific observed wet-day percentiles from the available baseline period.",
        threshold_rows,
    ...
    _write_table(
        wb.create_sheet("S14 ETCCDI Bias"),
    ...
    _write_table(
        wb.create_sheet("S15 ETCCDI Trend"),
