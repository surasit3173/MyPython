...
def _effective_n(values, lag_k=TREND_LAG_K):
    ...
    if n<8:
        return float(n), []
    if isinstance(lag_k,str) and lag_k.lower()=="adaptive":
        max_lag=min(ACF_MAX_LAG,max(1,int(np.sqrt(n))))
        ...
        for k in range(1,max_lag+1):
            ...
            if abs(rho)>=sig_thr:
                rhos.append((k,float(rho)))
    else:
        max_lag=min(int(lag_k),n-2)
        rhos=[]
        for k in range(1,max_lag+1):
            rho=_lag_autocorr(x,k)
            if np.isnan(rho): continue
...
# ===== v42 publication patches =====
from scipy.stats import spearmanr

def _lag_autocorr(x,k):
    a=np.asarray(x[:-k],dtype=float)
    b=np.asarray(x[k:],dtype=float)
...
def _effective_n(values, lag_k=TREND_LAG_K):
    ...
    if isinstance(lag_k,str) and lag_k.lower()=="adaptive":
        ...
        for k in range(1,max_lag+1):
            rho=_lag_autocorr(x,k)
            if not np.isnan(rho) and abs(rho)>=sig_thr:
                rhos.append((k,float(rho)))
    else:
        max_lag=min(int(lag_k),n-2)
        rhos=[(k,float(_lag_autocorr(x,k))) for k in range(1,max_lag+1)]
    cf=1.0
    for k,rho in rhos:
        w=((n-k)*(n-k-1)*(n-k-2))/(n*(n-1)*(n-2))
        cf+=2*w*rho
...
# ===== Observed-only publication workflow =====
OBS_OUTPUT_DIR="Obs_analysis"

def ensure_obs_dirs(base_out):
    obs_dir=Path(base_out)/OBS_OUTPUT_DIR
    obs_dir.mkdir(exist_ok=True)
    (obs_dir/"Figures").mkdir(exist_ok=True)
    return obs_dir


# ==== Publication workflow extensions v44 ====
def bootstrap_etccdi_ci(series, stat_func=np.nanmean, n_boot=BOOTSTRAP_N, ci=95):
    vals=np.asarray(series,dtype=float)
    vals=vals[~np.isnan(vals)]
    if len(vals)<10: return np.nan,np.nan,np.nan
    rng=np.random.default_rng(42)
    boots=[stat_func(rng.choice(vals,len(vals),replace=True)) for _ in range(n_boot)]
    lo=np.percentile(boots,(100-ci)/2)
    hi=np.percentile(boots,100-(100-ci)/2)
    return float(stat_func(vals)),float(lo),float(hi)

def build_mk_lag_sensitivity(ts,max_lag=ACF_MAX_LAG):
    rows=[]
    for k in range(1,max_lag+1):
        try:
            ac,_=spearmanr(ts[:-k],ts[k:])
        except:
            ac=np.nan
        rows.append({"lag":k,"rho":ac})
    return pd.DataFrame(rows)

def pseudo_validation_split(df):
    cal=df.loc[(df.index.year>=1981)&(df.index.year<=2000)].copy()
    val=df.loc[(df.index.year>=2001)&(df.index.year<=2014)].copy()
    return cal,val

def run_observed_analysis(obs_df,outdir="Obs_analysis"):
    Path(outdir).mkdir(exist_ok=True)
    xl=Path(outdir)/"Obs_analysis_summary.xlsx"
    with pd.ExcelWriter(xl,engine="openpyxl") as w:
        for stn in obs_df.columns:
            ts=obs_df[stn].dropna()
            build_mk_lag_sensitivity(ts).to_excel(w,sheet_name=f"{stn}_LagSensitivity",index=False)
            ann=ts.resample("YS").sum()
            m,lo,hi=bootstrap_etccdi_ci(ann.values)
            pd.DataFrame([{"station":stn,"mean":m,"ci_low":lo,"ci_high":hi}]).to_excel(
                w,sheet_name=f"{stn}_Bootstrap",index=False)
        cal,val=pseudo_validation_split(obs_df)
        pd.DataFrame({"cal_n":[len(cal)],"val_n":[len(val)]}).to_excel(w,sheet_name="PseudoValidation",index=False)
    return xl

# v45 placeholder combined publication package
OBS_DIR_NAME='Obs_analysis'
